...
- **Buildings and Processes** — a building takes time to become Ready. Extractors mine deposits, crafters run recipes, and a Hub enables routing and resource trade once it is an Active hub. A building's Mode selects its current output and changing it can burn $CPU. A Process commits the Cell for a bounded number of Batches and has no cancel action; crafting takes its inputs up front, while mining draws from the Cell's deposit as Cycles settle. Claiming an ended Process releases the Cell's process slot.
...
- **Logistics and markets** — ordinary Route endpoints are owned revealed Cells. Intermediate waypoints may be Virgin cells, owned Cells, or Active hubs; revealed foreign Cells without an Active hub are walls. `cpu_route_network` derives the legal graph from the complete current map and exports the component or components containing the chosen endpoints; `cpu_next_hops` is a one-hop alternative over the same map, not another stage of the export. A Route is submitted and judged as one complete chain. A Transport quote validates the chosen chain and current capacity but reserves neither. Goods become usable only after their Delivery arrives and is finalized. The internal resource market trades Warehouse units through Lots, Fills, Hub escrow, Eviction, and Lot returns. The external Cell market trades the Land NFTs themselves through listings and offers identified by `orderHash`; selling a Cell transfers its Cell-bound state and assets with it.
