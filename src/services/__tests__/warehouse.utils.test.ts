...
function capacityCell(balance: string, used: string, cap: string | null, incomingTransport = '0', lots = '0'): Cell {
    return projectCell(
        makeCell({
            resources: [
                makeResource({
...
describe('assessDestinationCapacity', () => {
    it('does not count incoming transport and lot reservations twice', () => {
        ...
        expect(assessDestinationCapacity(cell, RESOURCE_ID, 10n)).toEqual({
            fits: true,
            required: '10',
            free: '20',
        });
    ...
    it('treats an uncapped shelf as having unlimited room', () => {
        const cell = capacityCell('60', '80', null, '15', '5');

        expect(assessDestinationCapacity(cell, RESOURCE_ID, 20n)).toEqual({
            fits: true,
            required: '20',
            free: null,
        });
    ...
    it('does not invent a cap when the resource has no storage projection', () => {
    ...
    it('agrees with the projected full flag about whether room remains', () => {
        ...
        const capacity = assessDestinationCapacity(cell, RESOURCE_ID, 1n);
