 ...
 * as surely as what is on it now, so both count against the free figure. `emptyStorageCap` carries the config
 * rule when the map has no held row: null is uncapped WCPU, while every other missing cap resolves to zero.
 */
