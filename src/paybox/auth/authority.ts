...
import { PAYBOX_SCHEMA_VERSION, PAYBOX_TOKEN_REFRESH_WINDOW_MS } from '../constants.js';
import { payboxRefreshState, withPayboxRefreshState } from './coordinator.utils.js';
import type { PayboxAuthorityOptions } from './types.js';
...
export class PayboxAuthority implements WalletProvider {
    ...
    constructor(
        private readonly options: PayboxAuthorityOptions,
        ...
        this.wallet = this.createWallet(this.material, stored.credentialId, stored.address, this.generation);
    ...
    async authenticate(
        material: PayboxAuthMaterial | null,
        requestedCredentialId: string | null,
    ): Promise<PayboxAuthenticateResult> {
        if (material !== null) {
            return this.continueAuthentication(material, requestedCredentialId, this.generation);
        }
        if (this.isReady()) return this.authenticateReadyWallet(requestedCredentialId);
    ...
    private authenticateRestored(
        ...
        this.restoring = { credentialId, address, generation, promise: authentication };
    ...
    private async runRestoredAuthentication(
        ...
        try {
            const discovery = await this.options.sdk.listEligibleAutonomousEvmGrants(
                material.tokens,
                material.signingKey,
            );
    ...
    private continueAuthentication(
        ...
        this.continuing = { material, requestedCredentialId, generation, promise: continuation };
        return continuation;
    }
    ...
    private async continueWithMaterial(
        ...
        if (this.selection?.phase === PayboxSelectionPhase.Activating) {
            if (requestedCredentialId === null || this.selection.credentialId === requestedCredentialId) {
                return this.selection.promise;
            }
            throw new PayboxWalletSelectionError(PayboxErrorCode.WalletSelectionNotPending);
        }
        if (this.selection?.phase === PayboxSelectionPhase.AwaitingChoice) {
            if (requestedCredentialId === null) {
    ...
    private async activateGrant(
        ...
        const wallet = this.createWallet(material, grant.credentialId, address, generation);
    ...
    private async rotateTokens(material: PayboxAuthMaterial, generation: number): Promise<void> {
        ...
        const currentRecord = this.record(material);
        ...
        try {
            ...
            if (this.generation === generation) this.recoverRefreshFailure(currentRecord, refreshError);
            throw refreshError;
        }
        this.assertCurrent(generation);
        const refreshed: PayboxAuthMaterial = { tokens, signingKey: material.signingKey };
        try {
            this.options.storage.save(this.record(refreshed));
        } catch (error) {
    ...
    private recoverRefreshFailure(currentRecord: PayboxAuthRecord, refreshError: Error): void {
        ...
        try {
            ...
            this.refreshPersistenceError =
                error instanceof Error ? error : new Error('Paybox refresh recovery could not be persisted.');
            throw this.refreshPersistenceError;
        }
    }

    private record(material: PayboxAuthMaterial): PayboxAuthRecord {
        return {
            version: PAYBOX_SCHEMA_VERSION,
            tokens: material.tokens,
            signingKey: material.signingKey,
    ...
    private createWallet(
        ...
        return this.options.sdk.createWallet(
            material.tokens,
            material.signingKey,
            credentialId,
            address,
            this.walletAuthority(material, credentialId, address, generation),
        );
    }
    ...
    private walletAuthority(
        ...
        return {
            current: async () => {
                this.assertCurrent(authorityGeneration);
            ...
            invalidate: () => {
                if (this.generation !== authorityGeneration) return;
                try {
                    this.reset();
                } finally {
                    this.onInvalidated();
